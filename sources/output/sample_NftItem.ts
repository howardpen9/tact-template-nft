...
export async function NftItem_init(collection: Address, master: Address, item_index: BN, is_initialized: boolean) {
    ...
    __stack.push({ type: 'slice', cell: beginCell().storeAddress(collection).endCell() });
    ...
    let res = await executor.get('init_NftItem', __stack, { debug: true });
    if (res.debugLogs.length > 0) { console.warn(res.debugLogs); }
    let data = res.stack.readCell();
    return { code: codeCell, data };
}

export const NftItem_errors: { [key: string]: string } = {
    '2': `Stack undeflow`,
    '3': `Stack overflow`,
    '4': `Integer overflow`,
...
export class NftItem {
    ...
    async send(args: { amount: BN, from?: Address, debug?: boolean }, message: Transfer | OwnershipAssigned | GetStaticData | UpdateSubContent) {
        let body: Cell | null = null;
        if (message && typeof message === 'object' && !(message instanceof Slice) && message.$$type === 'Transfer') {
            body = packTransfer(message);
        }
        if (message && typeof message === 'object' && !(message instanceof Slice) && message.$$type === 'OwnershipAssigned') {
            body = packOwnershipAssigned(message);
        }
        if (message && typeof message === 'object' && !(message instanceof Slice) && message.$$type === 'GetStaticData') {
            body = packGetStaticData(message);
        }
        if (message && typeof message === 'object' && !(message instanceof Slice) && message.$$type === 'UpdateSubContent') {
            body = packUpdateSubContent(message);
        }
        ...
        try {
            ...
            if (e instanceof ExecuteError) {
                ...
                if (NftItem_errors[e.exitCode.toString()]) {
                    throw new Error(NftItem_errors[e.exitCode.toString()]);
                }
            }
            throw e;
        }
    ...
    async getGetNftData() {
        try {
            let __stack: StackItem[] = [];
            let result = await this.executor.get('get_nft_data', __stack, { debug: true });
            if (result.debugLogs.length > 0) { console.warn(result.debugLogs); }
            return unpackStackGetNftData(result.stack);
        } catch (e) {
            if (e instanceof ExecuteError) {
                if (e.debugLogs.length > 0) { console.warn(e.debugLogs); }
                if (NftItem_errors[e.exitCode.toString()]) {
    ...
    async getGetNftOwner() {
        try {
            let __stack: StackItem[] = [];
            let result = await this.executor.get('get_nft_owner', __stack, { debug: true });
            if (result.debugLogs.length > 0) { console.warn(result.debugLogs); }
